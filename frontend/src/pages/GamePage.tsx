...
function GamePage({ roomCode, players: initialPlayers, winsToMatch, onExit }: GamePageProps) {
  ...
  const availableCards = HAND_CARDS.filter((card) => (self?.cards[card.id] ?? 0) > 0);

  return (
    <div className="flex w-full max-w-md flex-1 flex-col items-center gap-6 px-6 py-10">
      <div className="text-center">
        <h1 className="text-2xl font-semibold text-text-h">가위바위보!</h1>
      ...
      <Card className="flex w-full flex-col items-center gap-8 py-10">
        <PlayZone
          ...
          opponentLabel={opponent ? nicknameOf(opponent.socketId) : "상대"}
              ...
              {availableCards.map((card) => (
                <div key={card.id} className="relative flex flex-col items-center gap-1">
                  <div className="relative">
                    <HandCard card={card} flipped onClick={() => handleSelectHand(card.id)} />
                    <span className="absolute -top-2 -right-2 flex h-6 w-6 items-center justify-center rounded-full bg-primary text-xs font-bold text-white shadow-panel">
                      {self?.cards[card.id] ?? 0}
                    </span>
                  </div>
                  {card.tag && <span className="max-w-24 text-center text-xs text-peach">{card.tag}</span>}
                </div>
              ))}
            </div>
          </div>
